...
Bootstrap a governed project under ~/Projects with Agent Forge contracts:
  - AGENTS.md
  - AGENTS.override.md  (Codex precedence stub, created only if not present)
  - CLAUDE.md
  - GEMINI.md
  - docs/CONOPS.md
  - docs/HANDOFF.md
...
# AGENTS.override.md — Codex 2026 precedence stub. Idempotent: only created
# if it doesn't already exist, so re-running bootstrap on an existing project
# never clobbers an operator's project-local Codex overrides.
if [[ ! -f "${TARGET_ROOT}/AGENTS.override.md" ]]; then
  cat > "${TARGET_ROOT}/AGENTS.override.md" <<EOF
<!-- Project-local Codex overrides. Codex reads this file BEFORE the parent-tree AGENTS.md. -->
<!-- This stub is created by bootstrap-project.sh and is NOT regenerated by the omni-factory sync. -->
