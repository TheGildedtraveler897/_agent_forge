...
- Append-first is paired with bounded decay. After a lesson's `Status:` becomes `promoted` and the named doctrine reference resolves, the entry is eligible for archival via `lesson-distiller` to `docs/archive/LESSONS_PROMOTED.md`. `docs/HANDOFF.md` follows the same pattern via `handoff-archiver` and `docs/archive/SPRINTS.md`. Distillation is reversible (the archive is the on-disk wisdom anchor) and operator-gated. The retention contract lives in `policies/distillation.json`.
